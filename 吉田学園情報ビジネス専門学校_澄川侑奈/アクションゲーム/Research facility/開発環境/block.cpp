...
////	ブロックのマクロ定義
////--------------------------------------------------------------------------------
//#define BLOCK_X					(60)		//ブロックX
//#define BLOCK_Y					(60)		//ブロックY
//
////-------------------------------------------------------------------------------
////	ブロックのグローバル変数
////-------------------------------------------------------------------------------
//LPDIRECT3DVERTEXBUFFER9 g_pVtxBlock = NULL;		//頂点バッファのポインタ
//LPDIRECT3DTEXTURE9 g_aBlockPicture = NULL;		//ブロックのテクスチャ(LP...はポインタのこと)
//
...
//
//	//テクスチャの読み込み
//	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/block001.png", &g_aBlockPicture);		//ブロックのテクスチャ
//
...
//		g_aBlock[nCntBlock].bBlock = false;
//		g_aBlock[nCntBlock].bGrab = false;
//	}
//
//	//頂点バッファの生成
...
//	g_pVtxBlock->Lock(0, 0, (void**)&pVtx, 0);
//
//	for (int nCntBlock = 0; nCntBlock < MAX_BLOCK; nCntBlock++)
//	{
//
//		//頂点座標
//		pVtx[0].pos = D3DXVECTOR3(g_aBlock[nCntBlock].pos.x, g_aBlock[nCntBlock].pos.y + BLOCK_Y, 0.0f);
//		pVtx[1].pos = D3DXVECTOR3(g_aBlock[nCntBlock].pos.x, g_aBlock[nCntBlock].pos.y, 0.0f);
//		pVtx[2].pos = D3DXVECTOR3(g_aBlock[nCntBlock].pos.x + BLOCK_X, g_aBlock[nCntBlock].pos.y + BLOCK_Y, 0.0f);
//		pVtx[3].pos = D3DXVECTOR3(g_aBlock[nCntBlock].pos.x + BLOCK_X, g_aBlock[nCntBlock].pos.y, 0.0f);
//
//		//rhwの設定
//		pVtx[0].rhw = 1.0f;
//		pVtx[1].rhw = 1.0f;
...
//
//	//テクスチャの開放
//	if (g_aBlockPicture != NULL)
//	{
//		g_aBlockPicture->Release();
//		g_aBlockPicture = NULL;
//	}
//}
//
////-------------------------------------------------------------------------------
...
//		g_aBlock[nCntBlock].posOld.y = g_aBlock[nCntBlock].pos.y;
//
//		if (pMark->bGrab == true)
//		{ //掴まれている時
//			
...
////				if (pBlock->bGrab == true && pMark->state == MARKERSTATE_CHACHING)
////				{
...
////
////					// 掴むor離すキーを押したとき
////					if (GetKeyboardTrigger(KEYINFO_CHACH) == true)
////					{
////						if (bHit == false)
////						{// 当たっていないとき
////							pBlock->fWidth = BLOCK_SIZE;								// サイズを元に戻す
////							pBlock->fHeight = BLOCK_SIZE;								// サイズを元に戻す
////							pBlock->pos.x = pMarker->pos.x - (pBlock->fWidth / 2.0f);	// 元の位置から少しずれているので中心座標の設定
////							pBlock->pos.y = pMarker->pos.y - (pBlock->fHeight / 2.0f);	// 元の位置から少しずれているので中心座標の設定
////							pMarker->state = MARKERSTATE_NORMAL;						// マーカーの状態をつかんでいないときにする
////							pBlock->bGrab = false;										// 掴んでいないときにする
////						}
////					}
////					else if (bHit == true)
////					{// 当たっているとき
...
////					}
////				}
////				else if (pBlock->bGrab == false && pMarker->state == MARKERSTATE_NORMAL)
...
////
////							pBlock->fWidth = BLOCK_SIZE / 2.0f;		// ブロックのサイズを小さくする
////							pBlock->fHeight = BLOCK_SIZE / 2.0f;	// ブロックのサイズを小さくする
////							pMarker->state = MARKERSTATE_CHACHING;	// マーカーの状態を掴んでいる状態にする
////							pBlock->bGrab = true;					// ブロックを掴まえている状態にする
////						}
////					}
////				}
////			}
...
//
//	//頂点バッファをデータストリームに設定
//	pDevice->SetStreamSource(0, g_pVtxBlock, 0, sizeof(VERTEX_2D));
//
//	//頂点フォーマットの設定
...
//		{
//			//テクスチャの設定
//			pDevice->SetTexture(0, g_aBlockPicture);
//
//			//ブロックの描画
...
//BLOCK * GetBlock(void)
//{
//	return &g_aBlock[0];	//ブロックの情報の先頭アドレスを返す
//}
				...
				if (pBlock->bChach == true && pMarker->state == MARKERSTATE_CHACHING)
				{
					...
					if (GetkeyboardTrgger(DIK_RETURN) == true)
					{
						...
						{// 当たっていないとき
							...
							PlaySound(SOUND_LABEL_SE_OFF);											//SEの追加(オフ)
