...
//	m_effect = CEffect::Create(D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(50.0f, 50.0f, 0.0f));
	...
	if (pFile != NULL)
	{
		...
		for (int nCountX = 0; nCountX < MAX_COLUMN; nCountX++)
		{
			...
			{
				//if (m_MapData[nCountY][nCountX] == 1)
				//{// Excelの数字が1の時
				//	// 敵の生成
				//	m_pEnemy = CEnemy::Create(D3DXVECTOR3(0.0f + (nCountX * ENEMY_WIDTH / 3.0f) + ENEMY_WIDTH / 2.0f,
				//		0.0f + (nCountY * ENEMY_HEIGHT / 3.0f) + ENEMY_HEIGHT / 2.0f,
				//		0.0f),
				//		ENEMY_SIZE, ENEMY_MOVE, ENEMY_SPEED);
				//}
