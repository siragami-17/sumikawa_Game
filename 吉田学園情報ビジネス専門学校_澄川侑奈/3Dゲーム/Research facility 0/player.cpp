...
#define MAX_SIZE_PLAYER_X		(10.0f)		// サイズ_x
#define MAX_SIZE_PLAYER_Y		(100.0f)	// サイズ_y
#define MAX_SIZE_PLAYER_Z		(50.0f)		// サイズ_z
...
//	g_player.pos.y--;
	...
	// オブジェクトとプレイヤーの当たり判定
	SetCollision(&g_player.pos, &g_player.posOld, g_player.size, &pObject->pos, pObject->size);
...
{
	// 床の中にいるとき
	if (g_player.pos.x > MAX_SIZE - MAX_SIZE_PLAYER_X ||
		g_player.pos.x < -MAX_SIZE - MAX_SIZE_PLAYER_X ||
		g_player.pos.z > MAX_SIZE   ||
		g_player.pos.z < -MAX_SIZE)
	{
		// 床から出たらプレイヤーの位置をposoldの位置に戻す
