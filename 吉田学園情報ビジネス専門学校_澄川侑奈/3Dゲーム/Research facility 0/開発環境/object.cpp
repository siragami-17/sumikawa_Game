...
#define MAX_SIZE_OBJECT_Z	(400.0f)		// サイズ_z
...
HRESULT InitObject(void)
{
	...
	// block構造体の初期化
	g_object.pos = D3DXVECTOR3(0.0f, 0.0f, 500.0f);											// 位置
...
void UpdateObject(void)
{
	...
	// プレイヤーとオブジェクトの当たり判定
	SetCollision(&pPlayer->pos, &pPlayer ->posOld, pPlayer->size, &g_object.pos, g_object.size);
}
...
{
	return &g_object;			// ブロック情報の先頭アドレスを返す
}